...
In the new editor, these are represented as **Overrides**, which change selected settings while inheriting the rest from the defaults. An override can apply to individual students or to student labels, which are reusable groups for things like sections or accommodations. Here's how they can be used to support the above scenarios:
...
Overrides for student labels are stored under `accessControl` in `infoAssessment.json`, which makes them easy to review and reuse across course copies. Overrides for individual students are stored in the database instead, so one-off accommodations and student identifying information don't become part of the course repository.
