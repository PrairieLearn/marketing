import { BlogMarkdownLayout, BlogImage } from "../../../../components/BlogMarkdownLayout";
...
Welcome to our first blog post! I started up this blog so that I could talk about all the developer tooling work we have been doing recently.
If you are not a developer, be warned -- this is highly technical stuff!
...
We have since been able to ship code with a lot less "nits" which has saved us a lot of review cycles (and time!).
...
we have [AI-powered Google Glass](https://www.meta.com/ai-glasses/meta-ray-ban-display/), but we aren't warning the developer if their code is going to crash?

We have spent a lot of time guiding PRs towards the "pit of success", and I wanted to share some of the improvements we made.
...
Python is a picture perfect example of dynamically-typed language -- it can crash for tons of reasons that are entirely non-obvious.
...
This is a core piece of our platform, and dates back over 10 years! This system has a couple drawbacks, one of which is a lack of standardized tooling for our exact setup.


To fix this, I created a question testing and accessibility suite that runs across all our 
questions in the [example course](https://github.com/PrairieLearn/PrairieLearn/tree/master/exampleCourse). It renders all the questions with a correct and incorrect input given,
and checks for issues in the question, and also accessibility violations. We use [`html-validate`](https://gitlab.com/html-validate/html-validate) for static accessibility testing.
...
As PrairieLearn develops, also will hit code paths on every deploy for elements that can help uncover regressions in the future.
...
We are a planned early adopter of [`postgres-language-server`](https://github.com/supabase-community/postgres-language-server) ([PR](github.com/prairielearn/prairielearn/pull/12682)). The maintainers there were able to provide a variety of [bug fixes and enhancements](https://github.com/supabase-community/postgres-language-server/issues?q=is:issue%20author:reteps) to get this to work for us, including `$named_parameter_support` and schema globs.
...
To allow for this, I finished implementing the full property list of JSONSchema and add support for `<details>` blocks for objects.

Using Zod has let us reuse these definitions across our TypeScript codebase, which has been incredibly useful.

### Typescript

We were able to finish converting all our our code from JavaScript to TypeScript.
...
We are still in the progress of progressively making our TypeScript configurations stricter.
...
-- this makes it much easier to iterate quickly. The time to first feedback (lints) is < 2 minutes no matter what you are working on. 
...
- Using a matrix build for our images. We are taking advantage of [registry caches](https://docs.docker.com/build/cache/backends/registry/) for builds that are consistently < 4 minutes (dominated by yarn install time).
...
If you liked this, I wrote [a similar post](https://stenger.io/blog/automation-sigpwny/) for the UIUC Cybersecurity club, `SIGPwny`, last year, 
focused more on automation.
