import {
  BlogMarkdownLayout,
  BlogImage,
  BlogQuote,
  BlogCalloutBox,
} from "../../../../components/BlogMarkdownLayout";

export const meta = {
  title: "Introducing AI grading",
  date: "2026-05-15",
  author: "Miguel Aenlle",
  tags: ["RELEASE"],
};

<div
  className="mb-5"
  style={{
    fontSize: "clamp(1.5rem, 3.5vw, 2.5rem)",
    fontWeight: 600,
    textAlign: "center",
    lineHeight: 1.1,
    margin: "2rem 0",
  }}
>
  <div style={{ color: "#1a3a8f" }}>AI grade hundreds of open ended</div>
  <div>
    <span style={{ color: "#1a3a8f" }}>submissions in </span>
    <span style={{ color: "#3b82f6" }}>minutes</span>
    <span style={{ color: "#1a3a8f" }}>, not </span>
    <span style={{ color: "#3b82f6" }}>hours</span>
    <span style={{ color: "#1a3a8f" }}>.</span>
  </div>
</div>

AI grading evaluates open-ended submissions, typed or handwritten, and scores them against your rubric in seconds. Grading hundreds of submissions now takes a single click, and it's available to all PrairieLearn instructors today.

AI grading is opt-in and transparent. AI grading decisions come with detailed explanations and are clearly distinguished from human grades. Course staff remain the final decision-makers on student grading.

### How it works

<div className="my-4">
  <div className="mb-5">
    <h4 className="fw-semibold mb-2">Collect submissions</h4>
    <div className="text-muted mb-3">
      Open-ended text or images from your students — handwritten work fully
      supported.
    </div>
    <div className="row g-3">
      <div className="col-md-6">
        <div className="border rounded bg-light p-5 text-center text-muted h-100 d-flex align-items-center justify-content-center">
          [ typed submission image ]
        </div>
      </div>
      <div className="col-md-6">
        <div className="border rounded bg-light p-5 text-center text-muted h-100 d-flex align-items-center justify-content-center">
          [ handwritten submission image ]
        </div>
      </div>
    </div>
  </div>
  <div className="mb-5">
    <h4 className="fw-semibold mb-2">Run AI grading</h4>
    <div className="text-muted mb-3">
      Click <strong>AI grading</strong> and select the model that fits your
      assessment.
    </div>
    <div className="border rounded bg-light p-5 text-center text-muted">
      [ model picker image ]
    </div>
  </div>
  <div className="mb-5">
    <h4 className="fw-semibold mb-2">Come back in a few minutes</h4>
    <div className="text-muted mb-3">
      Submissions are graded, with detailed reasoning attached to every
      decision.
    </div>
    <div className="border rounded bg-light p-5 text-center text-muted">
      [ graded results image ]
    </div>
  </div>
</div>

### Proven accuracy, backed by peer-reviewed research

We validated AI grading on thousands of real student submissions across multiple STEM courses at the University of Illinois Urbana-Champaign, which have been piloting the system since Fall 2025.

Across a range of open-ended questions and frontier models, rubric-item accuracy exceeded 99% when paired with clear student work and a well-aligned rubric. Instructors and TAs reported that AI grading was as accurate as human graders for their needs and saved enormous amounts of grading effort.

Read our published research on [image grading](#) and [text grading](#).

### Pick the right model for your assessment

AI grading lets you choose the model that fits your submissions, budget, and timeline.

<div className="row g-3 my-4">
  <div className="col-md-4">
    <div className="h-100 p-4 border rounded bg-light">
      <div className="fs-5 fw-semibold mb-2">GPT 5.4-mini</div>
      <div className="mb-0">
        Good default for most assessments. The most affordable option.
      </div>
    </div>
  </div>
  <div className="col-md-4">
    <div className="h-100 p-4 border rounded bg-light">
      <div className="fs-5 fw-semibold mb-2">GPT 5.4</div>
      <div className="mb-0">
        Best for text submissions where careful reasoning matters most.
      </div>
    </div>
  </div>
  <div className="col-md-4">
    <div className="h-100 p-4 border rounded bg-light">
      <div className="fs-5 fw-semibold mb-2">Gemini 3.1 Pro</div>
      <div className="mb-0">
        Best for handwriting and image-heavy submissions.
      </div>
    </div>
  </div>
</div>

#### Speed and costs of each model

Here were AI grading costs in two real PrairieLearn courses.

<div className="my-4">
  <div className="fw-semibold lh-sm mb-1">
    Numerical methods — text submissions
  </div>
  <div className="text-muted lh-sm mb-3">
    100 typed, paragraph-length responses graded with a rubric.
  </div>
  <div className="table-responsive">
    <table className="table">
      <thead>
        <tr>
          <th></th>
          <th className="text-end">Total cost</th>
          <th className="text-end">Total time</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td>GPT 5.4-mini</td>
          <td className="text-end">{"$0.20"}</td>
          <td className="text-end">10s</td>
        </tr>
        <tr>
          <td>GPT 5.4</td>
          <td className="text-end">{"$0.58"}</td>
          <td className="text-end">30s</td>
        </tr>
        <tr>
          <td>Gemini 3.1 Pro</td>
          <td className="text-end">{"$1.26"}</td>
          <td className="text-end">60s</td>
        </tr>
      </tbody>
    </table>
  </div>
</div>

<div className="my-4">
  <div className="fw-semibold lh-sm mb-1">
    Intro dynamics — image submissions
  </div>
  <div className="text-muted lh-sm mb-3">
    100 image submissions of handwritten math graded with a rubric.
  </div>
  <div className="table-responsive">
    <table className="table">
      <thead>
        <tr>
          <th></th>
          <th className="text-end">Total cost</th>
          <th className="text-end">Total time</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td>GPT 5.4-mini</td>
          <td className="text-end">{"$0.42"}</td>
          <td className="text-end">30s</td>
        </tr>
        <tr>
          <td>GPT 5.4</td>
          <td className="text-end">{"$1.59"}</td>
          <td className="text-end">60s</td>
        </tr>
        <tr>
          <td>Gemini 3.1 Pro</td>
          <td className="text-end">{"$4.88"}</td>
          <td className="text-end">2min 30s</td>
        </tr>
      </tbody>
    </table>
  </div>
</div>

### Direction and interpretability at every step

Every grade comes with a detailed explanation of the model's reasoning and its transcription of image submissions — so you can audit decisions, spot edge cases, and trust the results. AI grading works best as a fast first pass, with human review reserved for the cases that need it. Instructor-assigned grades always take precedence.

[image]

Want different behavior? Edit your rubric — the model follows your instructions.

### Pricing

Two ways to pay, depending on your preference.

<div className="row g-3 my-4">
  <div className="col-md-6">
    <div className="h-100 p-4 border rounded bg-light">
      <div className="fs-5 fw-semibold mb-3">PrairieLearn-managed</div>
      <div className="mb-0">
        Purchase credits with a credit card. PrairieLearn handles all
        communication with AI providers.
      </div>
    </div>
  </div>
  <div className="col-md-6">
    <div className="h-100 p-4 border rounded bg-light">
      <div className="fs-5 fw-semibold mb-3">Bring your own keys</div>
      <div className="mb-0">
        Use your own AI provider API keys, and costs are billed directly to you.
      </div>
    </div>
  </div>
</div>

### Get started

Three steps to grade your first assignment with AI:

1. Create a manually graded question with a rubric.
2. Fund your account under **Instance settings → AI Grading → Purchase credits**.
3. Open your assessment's **Manual grading** page, choose **AI grading → Grade all submissions**, and pick a model.

<BlogCalloutBox variant="info">
  More detailed information can be found in the [AI grading documentation](#).
  If you have questions, let us know on [Slack](/slack).
</BlogCalloutBox>

export default ({ children }) => (
  <BlogMarkdownLayout meta={meta}>{children}</BlogMarkdownLayout>
);
